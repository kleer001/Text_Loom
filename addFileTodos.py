...
def find_python_files(
    directory: str, max_depth: int = None, exclude_file: str = None
) -> List[str]:
    """
...
def read_existing_todos(
    todo_file: str,
) -> Tuple[Dict[str, Tuple[int, str, str]], List[str]]:
    ...
    with open(todo_file, "r") as file:
        ...
        for i, line in enumerate(content):
            if (
                line.startswith(TODO_MARKER) or line.startswith(DONE_MARKER)
            ) and line.strip().endswith(AUTO_TODO_MARKER):
                parts = line.strip()[:-1].split(
                    " *"
                )  # Remove AUTO_TODO_MARKER before splitting
                if len(parts) >= 2:
                    todo_text = " ".join(parts[:-1]).strip()[
                        4:
                    ...
                    if len(file_info_parts) == 3:
                        ...
                        line_number = line_number.split("_")[
                            -1
                        ]  # Remove 'Line_' prefix
                        ...
                        logging.warning(
                            f"Malformed auto-TODO entry at line {i+1}: {line.strip()}"
                        )
                else:
                    logging.warning(
                        f"Malformed auto-TODO entry at line {i+1}: {line.strip()}"
...
def update_todo_file(
    ...
    for line in content:
        if not (
            line.strip().startswith(TODO_MARKER)
            and line.strip().endswith(AUTO_TODO_MARKER)
        ) and not (
            line.strip().startswith(DONE_MARKER)
            and line.strip().endswith(AUTO_TODO_MARKER)
        ):
    ...
    for file_path, line_number, function_name, todo_text in todos:
        ...
        if relative_path in existing_todos:
            ...
            if existing_text.strip() != clean_todo_text or str(
            ...
            del existing_todos[
                relative_path
            ]  # Remove from existing_todos to mark as processed
        else:
            new_count += 1
            if not quiet:
...
def main(
    ...
    try:
        ...
        python_files = find_python_files(
            project_directory, max_depth, exclude_file=current_script
        )
        all_todos = []
        for file in python_files:
            all_todos.extend(extract_todos(file, quiet))
        new_count, updated_count = update_todo_file(
            all_todos, todo_file, section, quiet, project_directory
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Extract TODOs from Python files and update TODO.md"
    )
    parser.add_argument(
        "-d",
        "--directory",
    ...
    parser.add_argument(
        ...
        "--file",
        default="TODO.md",
        help="TODO file to update (default: TODO.md in current directory)",
    )
    parser.add_argument(
        "-s", "--section", help="Section in TODO.md to add new TODOs (optional)"
    )
    parser.add_argument(
        "-q", "--quiet", action="store_true", help="Quiet mode - only print summary"
    )
    parser.add_argument(
        "--max-depth",
        type=int,
        help="Maximum depth to search for Python files (default: no limit)",
    )
    parser.add_argument(
        "--log-level",
        default="INFO",
