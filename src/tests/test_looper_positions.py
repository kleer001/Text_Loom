...
def test_looper_position_initialization():
    ...
    Note: This simulates the API layer's position-setting behavior since position
    offset logic is handled in src/api/routers/nodes.py, not in src/core.
    """
    print("\n" + "="*60)
    ...
    # Simulate API layer position initialization (from src/api/routers/nodes.py)
    if looper._internal_nodes_created:
        parent_pos = looper._position
        looper._input_node._position = [parent_pos[0], parent_pos[1]]
        looper._output_node._position = [parent_pos[0] + LOOPER_OUTPUT_NODE_OFFSET_X, parent_pos[1]]
    ...
    expected_output_x = LOOPER_OUTPUT_NODE_OFFSET_X
    ...
    print(f"\n4. Verifying {LOOPER_OUTPUT_NODE_OFFSET_X}-pixel horizontal offset between nodes...")
    offset_x = output_node._position[0] - input_node._position[0]
    offset_y = output_node._position[1] - input_node._position[1]
    assert offset_x == LOOPER_OUTPUT_NODE_OFFSET_X, f"Expected x offset of {LOOPER_OUTPUT_NODE_OFFSET_X}, got {offset_x}"
    ...
    print("="*60)
    print(f"\nLooper node internal nodes are properly positioned with {LOOPER_OUTPUT_NODE_OFFSET_X}-pixel offset.")
    print()
